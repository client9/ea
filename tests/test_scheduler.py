...
def free_calendar(*emails) -> CalendarClient:
    ...
    return CalendarClient(
        fixture_data={"calendars": {email: {"busy": []} for email in emails}}
    )


def busy_calendar(
    email: str, busy_start: datetime, busy_end: datetime
) -> CalendarClient:
...
def test_free_when_no_conflicts():
    ...
    result = check_slot(
        ...
        WORKING_HOURS,
        PREFERRED_HOURS,
        free_calendar("me@example.com", "sarah@example.com"),
        TIMEZONE,
    )
    assert result.free is True
...
def test_busy_when_attendee_conflicts():
    ...
    result = check_slot(
        ...
        WORKING_HOURS,
        PREFERRED_HOURS,
        busy_calendar("me@example.com", start, end),
        TIMEZONE,
    )
...
def test_busy_block_partially_overlaps_slot():
    ...
    result = check_slot(
        ...
        WORKING_HOURS,
        PREFERRED_HOURS,
        busy_calendar("me@example.com", busy_start, busy_end),
        TIMEZONE,
    )
    assert result.free is False
