...
def free_calendar(*emails) -> CalendarClient:
    return CalendarClient(fixture_data={"calendars": {e: {"busy": []} for e in emails}})


def busy_at(email: str, iso_start: str, iso_end: str) -> CalendarClient:
    return CalendarClient(
        fixture_data={
            "calendars": {email: {"busy": [{"start": iso_start, "end": iso_end}]}}
        }
    )
...
def test_needs_confirmation_only_when_all_free_slots_are_after_hours():
    ...
    p = parsed(
        proposed_times=[
            {
                "text": "Thursday at 2pm",
                "datetimes": [THU_2PM_UTC],
            },  # preferred, but busy
            {
                "text": "Thursday at 7pm",
                "datetimes": [THU_7PM_UTC],
            },  # after-hours, free
        ]
    )
    ...
    calendar = CalendarClient(
        fixture_data={
            "calendars": {
                MY_EMAIL: {"busy": []},
                "sarah@example.com": {
                    "busy": [{"start": THU_2PM_UTC, "end": "2026-03-19T18:30:00Z"}]
                },
            }
        }
    )
