...
- **`ea/parser/meeting_parser.py`** — `parse_meeting_request(text, tz_name)` calls Claude (`claude-sonnet-4-20250514`) and returns structured JSON. Claude outputs plain-English time phrases (`normalized`); `parsedatetime` converts them to UTC ISO strings post-call. Callers see a `datetimes` list on each `proposed_times` entry.
...
- **`ea/poll.py`** — `run_poll(gmail, calendar, state, config, ...)` runs one full three-pass cycle. All dependencies are injected (testable without real APIs). Each thread's processing is wrapped in try/except — a crash on one thread logs the error and continues rather than aborting the cycle.
